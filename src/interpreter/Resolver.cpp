...
void Resolver::visitClassStmt(const Class &stmt) {
  ...
  if (stmt.superclass != nullptr &&
    ...
    Error::addError(stmt.superclass->name,
...
void Resolver::visitReturnStmt(const Return &stmt) {
  if (currentFunction == FUNCTION_NONE) {
    Error::addError(stmt.name,
                    "Resolvetime Error. Cannot return from top-level code.");
...
void Resolver::visitBreakStmt(const Break &stmt) {
  ...
  if (loop_nesting_level == 0) {
    Error::addError(stmt.keyword,
                    "Resolvetime Error. Can't break outside of a loop.");
...
void Resolver::declare(Token name) {
  ...
  if (scope.find(name.lexeme) != scope.end()) {
    Error::addError(name, "Resolvetime Error. Variable with the name '" +
                              name.lexeme + "' already exists in this scope");
    // lox::error(name.line,
...
void Resolver::resolveFunction(shared_ptr<Function> function,
                               FunctionType type) {
